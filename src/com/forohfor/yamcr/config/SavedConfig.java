package com.forohfor.yamcr.config;

import com.forohfor.yamcr.recognition.AreaRecognitionStrategy;
import com.forohfor.yamcr.recognition.AutoDetectAreaStrat;
import com.forohfor.yamcr.recognition.HashNarrowedRecogStrat;
import com.forohfor.yamcr.recognition.RecognitionStrategy;
import com.forohfor.yamcr.ui.StrategySelect;
import com.forohfor.yamcr.webcam.WebcamUtils;
...
public class SavedConfig
{
    ...
    /**
     * Initialize configuration in headless mode (no GUI dialogs).
     * Suitable for CLI applications.
     * If the config file doesn't exist, creates a default config with a 'yamcr-data' directory
     * in the current working directory.
     * 
     * @param configPath Path to config.json file
     * @throws IOException if config file cannot be read or written
     * @throws org.json.simple.parser.ParseException if config file is malformed
     */
    @SuppressWarnings("unchecked")
    public static void initHeadless(String configPath) throws IOException, org.json.simple.parser.ParseException
    {
        ...
        if (!f.exists())
        {
            System.out.println("Config file not found. Creating default config at: " + configPath);
            
            // Generate default path in current directory
            String defaultPath = Paths.get(System.getProperty("user.dir"), "yamcr-data").toString();
            ...
            System.out.println("Created default config with data path: " + defaultPath);
        }
        else
        {
        ...
        PATH = JSONUtil.getStringData(CONF_OBJECT, "path");
        DEBUG = JSONUtil.getBoolData(CONF_OBJECT, "debug");
        WRITE_BASICS_TO_SETS = JSONUtil.getBoolData(CONF_OBJECT, "write_basics_to_sets");
        
        // Validate that PATH is set
        if (PATH == null || PATH.isEmpty())
        {
            throw new IllegalStateException("Config file must specify 'path' for set data directory");
        }
        ...
        File setsDir = new File(PATH + "sets");
        File decksDir = new File(PATH + "decks");
        File customDir = new File(PATH + "custom");
        setsDir.mkdirs();
        decksDir.mkdirs();
        customDir.mkdirs();
        
        System.out.println("Data directories created/verified at: " + PATH);
